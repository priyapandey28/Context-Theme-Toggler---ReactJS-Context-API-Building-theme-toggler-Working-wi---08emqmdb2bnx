...
const LocalThemedBox = () => {
  ...
  return (
    ...
    >
      ...
      <p id="local-themed-text-container" className={`txt-${localLight?'light':'dark'}`}>
        This is some Text Inside Local Theme box
      </p>
      <button id="local-theme-toggler" 
      className={`btn btn-${localLight?'light':'dark'} txt-${localLight?'light':'dark'}`}
      onClick={()=>themeCtx.dispatch({type:'toggleLocalTheme'})}
      >Toggle local theme to {localLight?'dark':'light'}</button>
    </div>
  );
};
